...
function mungeWeatherResponse(weatherResponse) {
  ...
  const mungeForecasts = forecasts.map(forecast => {
    return {
  ...
  return mungeForecasts;
}

function mungeYelpResponse(yelpResponse) {
  const mungeYelpReview = yelpResponse.map(review => {
    return {
      name: review.name,
